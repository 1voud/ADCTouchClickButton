...
ADCTouchClickButton::ADCTouchClickButton(uint8_t buttonPin, int adcSamples, int adcLimit)
{
  _pin           = buttonPin;
...
ADCTouchClickButton::ADCTouchClickButton(uint8_t buttonPin, int adcSamples, int adcLimit, boolean activeType)
{
  _pin           = buttonPin;
...
void ADCTouchClickButton::createReference(int adcSamples) {
...
void ADCTouchClickButton::Update()
{

//  _btnState = digitalRead(_pin);  // current appearant button state
  int value = read();
  value -= touchRef;       //remove offset
  _btnState = !(value > limit);
